...
#            search_tool = SerperDevTool(n_results=10)
            ...
            knowledge_sources=[template, tool_output, search_output],  # <-- FIXED: added tool_output and search_output
            verbose=True,
            llm=llm,
            )
            ...
            task_fact_check = Task(
                description=(
                    ...
                    "Strictly ensure headers match the template verbatim, with no numbering or extra symbols."
                ),  # <-- CHANGED: as above, makes validation explicit, lists, and header enforcement -->
                expected_output="Fully verified set of threat intelligence ready for analytical correlation, with explicit field-by-field source verification log.",  # <-- ADDED: require field-by-field log
                agent=fact_checker,
                context=[task_research]  # <-- Ensures only Researcher output is fact-checked -->
            )
